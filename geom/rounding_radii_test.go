...
func TestRoundingRadii_NewAndProperties(t *testing.T) {
	...
	if !r.TopLeft().Equal(NewSize[Scalar](5, 5)) ||
		!r.TopRight().Equal(NewSize[Scalar](5, 5)) ||
		!r.BottomLeft().Equal(NewSize[Scalar](5, 5)) ||
		!r.BottomRight().Equal(NewSize[Scalar](5, 5)) {
...
func TestRoundingRadiiLTRB(t *testing.T) {
	...
	if !r.TopLeft().Equal(NewSize[Scalar](1, 2)) ||
		!r.TopRight().Equal(NewSize[Scalar](3, 2)) ||
		!r.BottomLeft().Equal(NewSize[Scalar](1, 4)) ||
		!r.BottomRight().Equal(NewSize[Scalar](3, 4)) {
...
func TestRoundingRadii_Scale(t *testing.T) {
	r := NewRoundingRadiiLTRB(1.0, 2.0, 3.0, 4.0)
	s := r.Scale(2.0)
	if !s.TopLeft().Equal(NewSize[Scalar](2, 4)) ||
		!s.TopRight().Equal(NewSize[Scalar](6, 4)) ||
		!s.BottomLeft().Equal(NewSize[Scalar](2, 8)) ||
		!s.BottomRight().Equal(NewSize[Scalar](6, 8)) {
...
func TestRoundingRadii_ScaleToFit(t *testing.T) {
	...
	if sumTop > bounds.Width()+Scalar(1e-5) || sumLeft > bounds.Height()+Scalar(1e-5) {
