...
func TestRoundingRadii_NewAndProperties(t *testing.T) {
	r := NewRoundingRadii[F32](5)
	if !r.TopLeft().Equal(NewSize[F32](5, 5)) ||
		!r.TopRight().Equal(NewSize[F32](5, 5)) ||
		!r.BottomLeft().Equal(NewSize[F32](5, 5)) ||
		!r.BottomRight().Equal(NewSize[F32](5, 5)) {
...
func TestRoundingRadiiLTRB(t *testing.T) {
	r := NewRoundingRadiiLTRB[F32](1, 2, 3, 4)
	if !r.TopLeft().Equal(NewSize[F32](1, 2)) ||
		!r.TopRight().Equal(NewSize[F32](3, 2)) ||
		!r.BottomLeft().Equal(NewSize[F32](1, 4)) ||
		!r.BottomRight().Equal(NewSize[F32](3, 4)) {
...
func TestRoundingRadii_IsEmpty(t *testing.T) {
	r := NewRoundingRadii[F32](0)
	if !r.IsEmpty() {
		t.Errorf("IsEmpty: should be true for all zero radii")
	}
	r2 := NewRoundingRadiiLTRB[F32](0, 1, 0, 0)
	if r2.IsEmpty() {
...
func TestRoundingRadii_Scale(t *testing.T) {
	r := NewRoundingRadiiLTRB[F32](1, 2, 3, 4)
	s := r.Scale(2)
	if !s.TopLeft().Equal(NewSize[F32](2, 4)) ||
		!s.TopRight().Equal(NewSize[F32](6, 4)) ||
		!s.BottomLeft().Equal(NewSize[F32](2, 8)) ||
		!s.BottomRight().Equal(NewSize[F32](6, 8)) {
		t.Errorf("Scale: scaling not correct")
	}
}

func TestRoundingRadii_ScaleToFit(t *testing.T) {
	r := NewRoundingRadii[F32](10)
	bounds := NewRect[F32](0, 0, 15, 20)
...
func TestRoundingRadii_Equal(t *testing.T) {
	r1 := NewRoundingRadiiLTRB[F32](1, 2, 3, 4)
	r2 := NewRoundingRadiiLTRB[F32](1, 2, 3, 4)
	r3 := NewRoundingRadiiLTRB[F32](1, 2, 3, 5)
	if !r1.Equal(r2) {
