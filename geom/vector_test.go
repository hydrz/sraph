...
func TestVector3Arithmetic(t *testing.T) {
	...
	t.Run("Div", func(t *testing.T) {
		result := v2.Div(v1)
		if !Eq(result.X, F32(4.0)) || !Eq(result.Y, F32(2.5)) || !Eq(result.Z, F32(2.0)) {
...
func TestVector3VectorOperations(t *testing.T) {
	...
	t.Run("Normalize", func(t *testing.T) {
		...
		if !Eq(normalized.X, expectedX) || !Eq(normalized.Y, expectedY) || !Eq(normalized.Z, F32(0.0)) {
	...
	t.Run("Zero vector normalize", func(t *testing.T) {
		...
		if !Eq(normalized.X, F32(0.0)) || !Eq(normalized.Y, F32(0.0)) || !Eq(normalized.Z, F32(0.0)) {
...
func TestVector3CrossProduct(t *testing.T) {
	...
	cross := v1.Cross(v2)
	// (1,0,0) × (0,1,0) = (0,0,1)
	if !Eq(cross.X, F32(0.0)) || !Eq(cross.Y, F32(0.0)) || !Eq(cross.Z, F32(1.0)) {
...
func TestVector3Lerp(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run("", func(t *testing.T) {
			...
			if !Eq(result.X, tt.expected.X) || !Eq(result.Y, tt.expected.Y) || !Eq(result.Z, tt.expected.Z) {
